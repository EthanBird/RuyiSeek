...
文件级持久化增量索引仍在后续迭代中；当前版本已经能跟随本地卷的挂载与卸载重建内存索引。Wayland 下不会绕过桌面安全模型伪造全局修饰键监听：窗口、搜索、单实例与托盘可用，双击 Ctrl 将等待 Portal 或 DDE 提供合规能力。
...
```bash
sudo apt install ./dist/ruyiseek_0.1.0-14_amd64.deb
# 或
sudo dpkg -i dist/ruyiseek_0.1.0-14_amd64.deb
```
...
```bash
bash packaging/deb/verify.sh dist/ruyiseek_0.1.0-14_amd64.deb
```
