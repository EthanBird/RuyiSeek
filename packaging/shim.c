 ...
 * musl-gcc, any vendored C object (e.g. libdbus) is compiled by
 * the system gcc, and the system ld still searches
 ...
 * overridden.
 *
 * This file is consumed by packaging/deb/build.sh, which compiles it
 * into packaging/deb/shim.o and lets .cargo/config.toml link the
 * result into every x86_64-unknown-linux-musl binary. ruyiseek-ui is
 * built for x86_64-unknown-linux-gnu and does not need these stubs.
 *
 * Lives at packaging/shim.c (one level out of $STAGING) so the .deb
 * packaging step (which walks the entire $STAGING tree) does not
 * accidentally ship the source into /usr/share/doc/ruyiseek/ or
 * elsewhere on end-user systems.
 */
